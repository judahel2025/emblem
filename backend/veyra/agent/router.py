"""The agent intent router — Veyra's "know what to do" brain.
...
def handle(command, context=None):
    ...
    # Task continuation: if Veyra asked for detail last turn, resume THAT task now with the
        ...
        if not clarified:
            ...
            if not c.get("clear", True) and c.get("question"):
                return {"intent": "code", "reply": c["question"],
                        "pending": {"intent": "code", "original": command}, "actions": []}
        sys = ("You are Veyra in coding mode. Write clean, correct, complete code for the "
    ...
    if intent == "search":
        ...
        sys = ("You are Veyra. Summarize these web results to answer the user clearly and "
    ...
    if intent == "document":
        ...
        sys = ("You are Veyra. Write the requested document fully, in clean Markdown with a "
...
SYS_BRIEF = ("You are Veyra — warm, upbeat, and conversational. Reply briefly, in 1-3 short "
