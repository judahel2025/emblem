...
def _cloud_conn():
    ...
    ci = vault.get_secret("veyra_pg") or os.environ.get("VEYRA_PG")
    if not ci:
        return None
    import psycopg
...
def tick():
    n = _tick_local()
    n += _tick_automations()
    if os.environ.get("VEYRA_CLOUD") == "1":
        n += _tick_cloud()
    return n
...
def _seed_monitor():
    """Ensure a daily product monitor exists (idempotent), so Veyra surfaces changes on her own."""
    try:
        existing = db.query("SELECT id FROM scheduled_tasks WHERE tool = 'monitor.check' "
        ...
        db.write(
            ...
            "VALUES ('Daily product monitor', 'monitor.check', ?, 86400, datetime('now','+120 seconds'), 36500, 'active')",
            (json.dumps({"period": "day"}),))
    except Exception:
        traceback.print_exc()
    # Estoppel activity watch — new support messages / signups → alerts Veyra speaks.
    try:
        existing = db.query("SELECT id FROM scheduled_tasks WHERE tool = 'estoppel.activity_check' "
                            "AND status IN ('active','paused') LIMIT 1")
        if not existing:
            db.write(
                "INSERT INTO scheduled_tasks (title, tool, args_json, every_secs, next_run, runs_total, status) "
                "VALUES ('Estoppel activity watch', 'estoppel.activity_check', '{}', 180, datetime('now','+60 seconds'), 200000, 'active')")
    except Exception:
        traceback.print_exc()
...
def start():
    ...
    _started = True
    _seed_monitor()
    threading.Thread(target=_loop, name="veyra-scheduler", daemon=True).start()
