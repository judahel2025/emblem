"""Resend email connector — Veyra sends real, branded email.

Two clearly-separated paths:
  • email.send  — OWNER-ONLY. Always delivers to the vaulted alert_to address. Used for
    alerts, reports, and emailing Master Judah his own generated documents. An injected
...
All mail is wrapped in a branded Veyra HTML template. Attachments supported.
"""
...
DEFAULT_DOMAIN = "veyra.thequaniac.com"
DEFAULT_TO = "alliesjude@gmail.com"

SENDERS = {
    "alert": ("Veyra Alerts", "alerts"), "report": ("Veyra Reports", "reports"),
    "briefing": ("Veyra", "briefing"), "security": ("Veyra Security", "security"),
    "billing": ("Veyra Billing", "billing"), "system": ("Veyra", "noreply"),
    "default": ("Veyra", "veyra"),
}
...
def _brand_wrap(subject, inner_html):
    return (
        ...
        '<div style="padding:16px 24px;background:#15110d">'
        '<span style="display:inline-block;width:26px;height:26px;border-radius:7px;background:#15110d;'
        'border:1px solid #e0a86a;text-align:center;line-height:25px;color:#e0a86a;font-weight:800;vertical-align:middle">V</span>'
        '<span style="color:#f3ead9;font-weight:700;letter-spacing:.3px;margin-left:10px;vertical-align:middle">Veyra</span>'
        '</div>'
        ...
        'Sent by Veyra · your AI operator · veyra.thequaniac.com</div>'
        '</div></div>'
    )
...
@tool("email.send", Tier.SAFE,
      "Email Master Judah (owner-only; alerts, reports, or attach a generated document)",
      network=True)
