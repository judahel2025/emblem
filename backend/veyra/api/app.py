"""Veyra kernel-backed API (FastAPI).
...
_log = logging.getLogger("veyra.app")

app = FastAPI(title="Veyra Core", version="0.2.0")


# --- Background inbound-email poller -----------------------------------------
# Resend's webhook only fires for outbound events. For inbound emails (sent TO
# veyra@veyra.thequaniac.com) we poll their receiving API every 60 s.

_POLL_INTERVAL = 20   # seconds — near-real-time fallback between webhook fires

async def _inbound_poll_loop():
    await asyncio.sleep(10)   # brief startup delay
    while True:
        try:
            from ..connectors import mailroom
            result = mailroom.poll_and_store()
            if result.get("new", 0):
                _log.info("inbound poll: %d new email(s) stored", result["new"])
        except Exception as exc:
            _log.warning("inbound poll error: %s", exc)
        await asyncio.sleep(_POLL_INTERVAL)


app.add_middleware(
    CORSMiddleware,
    allow_origin_regex=r"(http://(127\.0\.0\.1|localhost)(:\d+)?)|(https?://tauri\.localhost)|(tauri://localhost)|(https://[a-z0-9\-]+\.veyra-app\.pages\.dev)|(https://veyra-app\.pages\.dev)",
    allow_methods=["*"],
    allow_headers=["*"],
)
...
_API_TOKEN = _os_auth.environ.get("VEYRA_API_TOKEN")
...
async def _auth_gate(request, call_next):
    ...
    per user. Public paths (/api/health, /api/mail/inbound) skip auth. On the local
...
async def _startup():
    ...
    asyncio.create_task(_inbound_poll_loop())
...
@app.get("/api/health")
def health():
    # Public + unauthenticated: reveal NOTHING about which AI/model/provider powers Veyra.
    ...
    try:
        ...
        ready = False
    return {"name": "Veyra", "status": "online", "ready": ready}
...
def send_email_draft(eid: int):
    """Human-initiated send: Master Judah reviewed the draft and clicked Send."""
    if not _ctx.is_owner():
        return _owner_404()
...
# --- master-password lock (single user = Master Judah; opt-in) --------------------
...
def chat(body: ChatIn):
    if not _ctx.is_owner():
        return _owner_404()
    from ..agent import brain
    sys = ("You are Veyra, Master Judah's coding copilot. Give correct, complete, working code "
...
# --- the brain: which LLM provider/model powers Veyra ----------------------------
...
# --- mailroom (inbound receive via Resend webhook) --------------------------------
@app.post("/api/mail/inbound")
async def mail_inbound(request: Request):
    """Real-time inbound email endpoint — called by Resend when a new email arrives.

    No auth required: inbound bodies are always tagged [untrusted] and never
    auto-executed, so open access is safe. The auth middleware already exempts
    this path from the bearer-token check.

    Handles both JSON and multipart/form-data payloads (Resend sends JSON;
    some routing proxies send form-data).
    """
    from ..connectors import mailroom
    import re as _re

    # --- parse payload (JSON or form-data) ---
    ...
    _log.info("inbound webhook body keys: %s", list(body.keys()) if body else "(empty)")

    # Resend inbound payload shapes:
    #   { "type": "email.received", "data": { "from": ..., "to": [...], "subject": ..., "text": ..., "html": ... } }
    #   OR flat:  { "from": ..., "to": [...], "subject": ..., "text": ..., ... }
    email_data = (body.get("data") if isinstance(body.get("data"), dict) else None) or body
    from_addr = str(email_data.get("from") or email_data.get("sender") or "")
    to_field  = email_data.get("to") or email_data.get("recipient") or []
    to_addr   = (to_field[0] if isinstance(to_field, list) else str(to_field)) or ""
    subject   = str(email_data.get("subject") or "")
    text_body = str(email_data.get("text") or email_data.get("plain") or "")
    html_body = str(email_data.get("html") or "")
    resend_id = str(email_data.get("id") or email_data.get("messageId") or "")

    import re as _re
    if not text_body and html_body:
        text_body = _re.sub(r"<[^>]+>", " ", html_body)
        text_body = _re.sub(r"\s{2,}", " ", text_body).strip()

    if from_addr and subject:
        # Direct store — no API round-trip, instantaneous
        mid = mailroom.store_inbound(from_addr, to_addr, subject, text_body, html_body)
        if resend_id:
            mailroom.mark_hwm(resend_id)   # prevent the poller from double-inserting
        _log.info("inbound webhook: stored mid=%d from=%s subject=%r", mid, from_addr, subject)
        stored_direct = True

    # Always kick a background poll as a safety net
    asyncio.create_task(asyncio.to_thread(mailroom.poll_and_store))
    return {"ok": True, "stored_direct": stored_direct}


@app.post("/api/mail/sync")
def mail_sync():
    """Full backfill: pull all emails from Resend and upsert into mail_messages."""
    if not _ctx.is_owner():
        return _owner_404()
    from ..connectors import mailroom
    result = mailroom.sync_from_resend()
...
_inbox_undo_buffer: list = []      # [{...mail_messages row...}, ...]
_inbox_undo_expires: float = 0.0   # epoch seconds


@app.post("/api/inbox/delete_all")
def inbox_delete_all():
    """Delete every row in mail_messages and return the count.
    Keeps a 30-second in-memory undo buffer.
    """
    if not _ctx.is_owner():
        return _owner_404()
    global _inbox_undo_buffer, _inbox_undo_expires
    from ..kernel import data as db
    # Snapshot all rows for undo
    rows = db.query("SELECT * FROM mail_messages", ())
    count = len(rows)
    db.write("DELETE FROM mail_messages", ())
    _inbox_undo_buffer  = rows
    _inbox_undo_expires = _time.time() + 30
    # Audit log
    try:
        audit.record("judah", "inbox.delete_all", "owner",
                     {"rows": count}, "ok", f"{count} rows deleted")
    except Exception:
        pass
    return {"ok": True, "deletedCount": count}


@app.post("/api/inbox/undo")
def inbox_undo():
    """Restore the last bulk delete if within the 30-second window."""
    if not _ctx.is_owner():
        return _owner_404()
    global _inbox_undo_buffer, _inbox_undo_expires
    from ..kernel import data as db
    if not _inbox_undo_buffer or _time.time() > _inbox_undo_expires:
        return _JSONResponse({"ok": False, "error": "Undo window expired"}, status_code=409)
    cols = ("direction", "from_addr", "to_addr", "subject",
            "text_body", "html_body", "status")
    for row in _inbox_undo_buffer:
        db.write(
            "INSERT INTO mail_messages (direction, from_addr, to_addr, subject, "
            "text_body, html_body, status) VALUES (?, ?, ?, ?, ?, ?, ?)",
            tuple(row.get(c, "") for c in cols),
        )
    restored = len(_inbox_undo_buffer)
    _inbox_undo_buffer  = []
    _inbox_undo_expires = 0.0
    try:
        audit.record("judah", "inbox.undo", "owner",
                     {"rows": restored}, "ok", f"{restored} rows restored")
    except Exception:
        pass
...
def mail_list(status: str = "unread", limit: int = 50):
    if not _ctx.is_owner():
        return _owner_404()
    from ..connectors import mailroom
    return {"items": mailroom.listing(status, limit), "unread": mailroom.unread_count()}


@app.get("/api/mail/messages/{mid}")
def mail_get(mid: int):
    if not _ctx.is_owner():
        return _owner_404()
    from ..connectors import mailroom
    msg = mailroom.get_one(mid)
    if not msg:
        return _JSONResponse({"error": "not found"}, status_code=404)
    return msg
...
def mail_read(mid: int):
    if not _ctx.is_owner():
        return _owner_404()
    from ..connectors import mailroom
    mailroom.mark_read(mid)
    return {"ok": True}


@app.post("/api/mail/messages/{mid}/archive")
def mail_archive(mid: int):
    if not _ctx.is_owner():
        return _owner_404()
    from ..connectors import mailroom
    mailroom.archive(mid)
    return {"ok": True}


class BulkMailIn(BaseModel):
    ids: list[int] = []


@app.post("/api/mail/bulk/read")
def mail_bulk_read(body: BulkMailIn):
    if not _ctx.is_owner():
        return _owner_404()
    from ..connectors import mailroom
    if not body.ids:
        return {"ok": False, "error": "No IDs provided"}
    count = mailroom.bulk_mark_read(body.ids)
    return {"ok": True, "marked_read": count}


@app.post("/api/mail/bulk/archive")
def mail_bulk_archive(body: BulkMailIn):
    if not _ctx.is_owner():
        return _owner_404()
    from ..connectors import mailroom
    if not body.ids:
        return {"ok": False, "error": "No IDs provided"}
    count = mailroom.bulk_archive(body.ids)
    return {"ok": True, "archived": count}


@app.post("/api/mail/messages/{mid}/suggest-reply")
def mail_suggest_reply(mid: int):
    if not _ctx.is_owner():
        return _owner_404()
    from ..connectors import mailroom
    from ..agent import brain
    ...
    if not msg:
        return _JSONResponse({"error": "not found"}, status_code=404)
    clean = (msg.get("text_body") or "").replace("[untrusted] ", "", 1).strip()
    prompt = (
        f"Draft a concise, professional reply to this email.\n\n"
        f"From: {msg.get('from_addr', '')}\n"
        f"Subject: {msg.get('subject') or '(no subject)'}\n"
        f"Body:\n{clean[:4000]}\n\n"
        "Write only the reply body — no subject line, no headers. Sign off as Judah."
    )
    result = brain.chat(prompt, max_tokens=600)
    return {
...
# --- alerts (Veyra comes alive when something happens) ---------------------------
@app.get("/api/alerts")
def alerts_unseen():
    from .. import alerts
...
# --- improvements (Veyra's "teach me" log; operator surface) ----------------------
