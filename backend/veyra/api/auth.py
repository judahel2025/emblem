...
Two ways to be authorized:
  ...
  2. The service principal: `Authorization: Bearer <VEYRA_API_TOKEN>` — the server-set token
     used by the heartbeat/scheduler and admin calls. Cannot be spoofed by request input.

On the local PC build (no SUPABASE_JWT_SECRET, no VEYRA_API_TOKEN) everything is open and the
...
_SERVICE_TOKEN = os.environ.get("VEYRA_API_TOKEN", "")
_SUPABASE_URL = os.environ.get("SUPABASE_URL", "").rstrip("/")
_ANON_KEY = os.environ.get("SUPABASE_ANON_KEY", "") or os.environ.get("VEYRA_SUPABASE_ANON", "")

# Paths that never require auth.
_PUBLIC = {"/api/health", "/api/mail/inbound"}
