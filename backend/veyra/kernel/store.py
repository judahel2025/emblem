...
The `veyra_pg` vault secret now points to the scoped `veyra_rw` role:
...
def _cs():
    cs = os.environ.get("VEYRA_PG")
    if cs:
        return cs
    try:
        from . import vault
        return vault.get_secret("veyra_pg")
    except Exception:
        return None
...
def init() -> None:
    ...
    Calling this against the scoped `veyra_rw` role will raise a permission
