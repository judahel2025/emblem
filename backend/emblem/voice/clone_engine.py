...
ROOT/backend/emblem/voice/samples/owner.wav to clone the owner voice.
...
from ..kernel.paths import ROOT, EMBLEM_HOME

_MODEL_DEFAULT  = str(ROOT / "models" / "xtts_v2")
_SAMPLE_DEFAULT = str(ROOT / "backend" / "emblem" / "voice" / "samples" / "owner.wav")
_CACHE_DEFAULT  = str(EMBLEM_HOME / "voice_cache")
...
def voices():
    return [{"id": "owner", "name": "the owner (cloned)",
             "gender": "Male", "locale": "en-US"}]


def synth(text: str) -> bytes:
    """Return 24 kHz mono WAV bytes in the owner's voice. Caches by text hash."""
