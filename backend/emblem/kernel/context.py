...
Multi-user Emblem threads the current user through a contextvar instead of adding a
...
_current_user: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "emblem_user_id", default=None
)
_current_actor: contextvars.ContextVar[str] = contextvars.ContextVar(
    "emblem_actor", default="user"
)

# When no EMBLEM_API_TOKEN / cloud auth is configured we're the local single-user build.
...
_OWNER_USER_ID = os.environ.get("EMBLEM_OWNER_USER_ID", "")
...
def is_owner() -> bool:
    ...
    else so the client (and DevTools) never learns which AI or tools power Emblem."""
    if not _MULTIUSER:
        return True
