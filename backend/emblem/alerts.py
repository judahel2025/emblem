"""Alerts — the feed that lets Emblem come alive on her own.

When something happens (a new support message, a signup, a product change), a connector raises
an alert here. The UI polls unseen alerts and, when one appears, Emblem speaks up unprompted and
asks whether to act. Also used to (optionally) email the operator the same alert.

Multi-user: every alert belongs to a user. Connector/heartbeat alerts (system
watchers) default to the admin/owner account; user-originated alerts
...
def raise_alert(kind="info", title="", body="", data=None, email=False, user_id=None):
    ...
    (system events). Request-context callers stamp that user."""
    ...
    if email:
        try:
            ...
            _k.execute_tool("email.send", {"subject": f"Emblem · {title}", "body": body,
