// /api/voice/live — browser WebSocket ⇄ Gemini Live, relayed by the Worker.
// Two modes:
//   chat        — Emblem's everyday realtime voice
//   onboarding  — Emblem MEETS a new user: she speaks first, interviews them
//                 naturally (name → work → what to help with → tone), then calls
//                 save_profile — the Worker writes profile + memories to D1 and
//                 tells the client it's done.
//
// The client sees only generic frames ({type: audio|caption|status}) — never the
// provider. Payload shapes mirror the proven Python bridge exactly.
//
// Pattern note: the upstream socket MUST be fully established inside the request
// lifecycle (before returning the 101). Backgrounding the connect — even under
// waitUntil — gets the in-flight fetch reaped ("Network connection lost").
...
async function saveProfile(env: Env, userId: string, a: Record<string, unknown>) {
...
export async function voiceRelay(req: Request, env: Env, _ctx: WaitUntil): Promise<Response> {
  if (req.headers.get("Upgrade")?.toLowerCase() !== "websocket") {
    return new Response("expected websocket", { status: 426 });
  }
  const url = new URL(req.url);
  const mode = url.searchParams.get("mode") === "onboarding" ? "onboarding" : "chat";
  const userId = await userFromToken(url.searchParams.get("token") || "", env);

  const pair = new WebSocketPair();
  const [client, server] = Object.values(pair) as [WebSocket, WebSocket];
  server.accept();

  const deny = (state: string) => {
    try { server.send(JSON.stringify({ type: "status", state })); server.close(); } catch { /* closed */ }
    return new Response(null, { status: 101, webSocket: client });
  };

  if (!userId || !env.GEMINI_KEY) return deny("unavailable");

  // Connect upstream NOW, inside the request lifecycle.
  let upstream: WebSocket | null = null;
  try {
    const res = await fetch(
      `https://${HOST}/ws/google.ai.generativelanguage.v1beta.GenerativeService.BidiGenerateContent?key=${env.GEMINI_KEY}`,
      { headers: { Upgrade: "websocket" } });
    upstream = res.webSocket;
  } catch { upstream = null; }
  if (!upstream) return deny("unavailable");
  upstream.accept();
    ...
    void (async () => {
      try {
        ...
        if (frame.setupComplete !== undefined && !ready) {
          ready = true;
          server.send(JSON.stringify({ type: "status", state: "ready" }));
          if (mode === "onboarding") {
        ...
        for (const p of sc.modelTurn?.parts || []) {
          ...
          if (inline?.data) server.send(JSON.stringify({ type: "audio", data: inline.data }));
        }
        if (sc.outputTranscription?.text) {
          server.send(JSON.stringify({ type: "caption", who: "assistant", text: sc.outputTranscription.text }));
        }
        if (sc.inputTranscription?.text) {
          server.send(JSON.stringify({ type: "caption", who: "user", text: sc.inputTranscription.text }));
        }
        if (sc.interrupted) server.send(JSON.stringify({ type: "status", state: "interrupted" }));
        if (sc.turnComplete) server.send(JSON.stringify({ type: "status", state: "turn_complete" }));

        if (frame.toolCall?.functionCalls?.length) {
          for (const fc of frame.toolCall.functionCalls) {
            if (fc.name === "save_profile") {
              await saveProfile(env, userId, fc.args || {});
              up.send(JSON.stringify({ tool_response: {
                function_responses: [{ id: fc.id, name: fc.name, response: { ok: true } }] } }));
              server.send(JSON.stringify({ type: "status", state: "onboarded" }));
            }
          }
        }
  ...
  up.addEventListener("close", () => {
    try { server.send(JSON.stringify({ type: "status", state: "ended" })); server.close(); } catch { /* closed */ }
  });
  ...
  server.addEventListener("message", (ev) => {
    try {
      ...
      if (msg.type === "end") { up.close(); server.close(); }
      else if (msg.type === "text" && msg.text) {
        up.send(JSON.stringify({ client_content: {
          turns: [{ role: "user", parts: [{ text: String(msg.text) }] }],
          turn_complete: true } }));
      }
    } catch { /* ignore bad client frame */ }
  });
