<script>
  ...
  // surfaces any pending approval right here so voice mode is never a dead end, // tap it, or just say "yes"/"approve" or "no"/"decline" while it's showing.
  ...
  async function decideVoice(approved) {
    ...
    // The real narrated confirmation lands in the chat thread a beat later, // speak it too, so voice mode hears the outcome, not just sees it.
    setTimeout(() => {
